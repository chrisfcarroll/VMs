...
ssh-copy-id $target
scp netbsd-* $target:
ssh $target 'sudo mv /home/\$SU_FROM/*.sh \$HOME'
...
echo "Done.
***
Note these scripts assume a standard install BSD with these choices made during setup:
***
- network working
- sshd enabled
- NetBSD: pkgin enabled (and optionally, pkgsrc enabled)
- OpenBSD: pkg_add working
- ssh to root is not possible
- added user as member of wheel

and that you have manually logged in as root at the console to:

- pkg_add sudo
- visudo to uncomment the line %wheel  ALL=(ALL) NOPASSWD: SETENV: ALL
"
